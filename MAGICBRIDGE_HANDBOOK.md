...
**Stealth panel redesign + Funnel fix (2026-07-09, commits `a486f32`, `72f9f88`, `eebfb64`, `9a0aec0`):**
...
- **Fixed — Tailscale Funnel toggle silently claimed success, then (once fixed to report reality) turned out to hang.** See §11 issues table for the full three-bug chain — fire-and-forget Popen, a `tailscale funnel` CLI quirk where it hangs instead of exiting when Funnel isn't tailnet-enabled, and a `--remove` flag that was never real. One manual step still required on Raj's end (visit the tailnet enable URL once).
...
| — | Tailscale Funnel toggle | **Fixed 2026-07-09** (commits `eebfb64`, `9a0aec0`) — three stacked bugs: (1) fire-and-forget Popen always claimed success, (2) the real `tailscale funnel 443` command hangs instead of exiting when Funnel isn't enabled for the tailnet, even with `--bg --yes`, even as root — now polls stdout instead of blocking on exit so the real answer surfaces in <1s, (3) `tailscale funnel --remove` was never a valid flag on this tailscale version, silently failing since day one — now uses `tailscale funnel reset`. **Still needs one manual step**: Funnel itself isn't turned on for this tailnet yet — visit `https://login.tailscale.com/f/funnel?node=n8M7MxqUbe11CNTRL` once to enable it, then the toggle will work end-to-end |
