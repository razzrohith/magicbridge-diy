...
#    3. hands off to WiFi provisioning: "Join Setup-XXXX" (per-unit AP name)
#
...
sleep 6
# Match the "connected" PREFIX like mb-provision.sh, so "connected (site only)"
# (a normal air-gapped / no-internet LAN state) counts as connected here too.
if nmcli -t -f STATE general 2>/dev/null | grep -qE '^connected'; then
    clear_oled
    echo "[$(date)] network present - normal operation"
else
    # Name the SAME per-unit AP mb-provision.sh will raise ("Setup-XXXX" from this
    # unit's machine-id), derived identically. The old static "MagicBridge-Setup"
    # text was simply wrong for the ~40s before mb-provision overwrites the OLED,
    # and it leaked the product name over the air. machine-id is already
    # regenerated by this point, so both scripts land on the same suffix.
    ...
    oled "@WIFI" "Join WiFi hotspot:" "Setup-${_ap_sfx}"
